...
1. [システム概要](#2-システム概要)
2. [インストール手順](#3-インストール手順)
3. [設定方法](#4-設定方法)
4. [基本的な使用方法](#5-基本的な使用方法)
5. [API使用方法](#6-api使用方法)
6. [GUI使用方法（Streamlitチャットインターフェース）](#8-gui使用方法streamlitチャットインターフェース)
...
- **OpenAI API Key** または **Gemini API Key**: LLMを使用するため（どちらか一方）
...
`--reload` オプションを付けると、コード変更時に自動的に再起動されます。
...
`examples/example_api_usage.py` を参考に、PythonからAPIを使用できます：
...
## 8. GUI使用方法（Streamlitチャットインターフェース）

### 8.1 概要

Streamlitを使用したWebベースのチャットインターフェースを提供します。ブラウザから直感的にリサーチエージェントを操作できます。

### 8.2 起動方法

#### 8.2.1 前提条件

1. **APIサーバーの起動**: GUIを使用するには、まずAPIサーバーを起動する必要があります。
...
#### 8.2.2 GUIアプリケーションの起動

別のターミナルでGUIアプリケーションを起動します：

```bash
# ターミナル2: GUIアプリケーションを起動
streamlit run examples/api_usage_with_chat_gui.py
```

起動すると、以下のメッセージが表示され、ブラウザが自動的に開きます：
...
| 変数名 | 必須 | デフォルト値 | 説明 |
|--------|------|------------|------|
| `LLM_PROVIDER` | いいえ | `openai` | 使用するLLMプロバイダー（`openai` または `gemini`） |
