...
class UI {
    ...
    displayResearchResult(result, researchId) {
        ...
        if (result.report && result.report.sources && result.report.sources.length > 0) {
            content += `## 📚 参照ソース (${result.report.sources.length}件)\n\n`;
            result.report.sources.forEach((source, index) => {
                content += `### ${index + 1}. ${source.title || 'N/A'}\n\n`;
                content += `- **URL**: [${source.url || 'N/A'}](${source.url || '#'})\n`;
                if (source.summary) {
        ...
        if (content.trim()) {
            ...
            // コンテンツが空の場合はエラーメッセージを表示
            this.clearProgress();
            this.addMessage('assistant', '⚠️ リサーチ結果が空です。', researchId);
        }
    }
