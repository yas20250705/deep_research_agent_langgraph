...
# GUI要件定義書：LangGraph搭載 自律型リサーチエージェント

## 1. プロジェクト概要

* **対象システム:** LangGraph搭載 自律型リサーチエージェントのGUI層。
* **目的:** ユーザーがテーマを入力し、リサーチエージェント（検索・執筆・レビュー）を操作し、レポートの生成・ダウンロードや参照ソースの確認を行えるWebベースのGUIを提供する。
* **GUI形態:** （1）**HTML GUI**（`gui/` 配下の HTML/CSS/JavaScript、API連携、推奨）、（2）**Streamlit GUI**（`examples/api_usage_with_chat_gui.py` を用意した場合に `run_gui.py` / `start_gui.bat` で起動可能）。
* **主要な価値:** 高速なレスポンス（ストリーミング）、直感的な操作性、過去のリサーチ履歴の管理、およびリッチなテキスト表示（Markdown/コード）を実現する。
