...
├── requirements.txt
├── pyproject.toml
├── .env_example                       # 環境変数雛形（.env にコピーして使用）
├── run_api_server.py                  # APIサーバー起動
├── run_gui.py                         # Streamlit GUI 起動
├── start_api_server.bat               # APIサーバー起動（Windows）
├── start_gui.bat                      # Streamlit GUI 起動（Windows）
├── start_html_gui.bat                 # HTML GUI 起動（Windows）
├── deep_research_agent_langgraph.bat  # API + HTML GUI 一括起動（Windows）
│
├── src/
│   ├── __init__.py
│   ├── api/                           # FastAPI
│   │   ├── __init__.py
│   │   ├── main.py                    # ルート・アプリ
│   │   ├── middleware.py              # CORS・認証等
│   │   ├── research_manager.py        # リサーチ実行管理
│   │   ├── schemas.py                 # リクエスト/レスポンススキーマ
│   │   └── streaming.py              # SSE ストリーミング
│   │
│   ├── config/
│   │   ├── __init__.py
...
│   ├── schemas/
│   │   ├── __init__.py
│   │   └── data_models.py            # データモデル（ResearchPlan, SearchResult, ResearchReport 等）
│   │
│   ├── tools/
│   │   ├── __init__.py
...
│       ├── error_handler.py          # ノード用エラーハンドリング
│       ├── human_in_loop.py           # 人間介入
│       ├── llm_factory.py             # LLM生成（OpenAI / Gemini / Mock）
│       ├── logger.py                  # ロギング設定
│       ├── mock_llm.py                # Mock LLM（テスト用）
│       ├── parallel.py                # 並列実行
│       ├── pdf_generator.py           # 参照ソースPDF生成
│       ├── profiler.py                # プロファイリング
│       ├── retry.py                   # リトライ
│       ├── security.py                # 入力検証等
│       ├── summarizer.py              # URL要約
│       └── title_generator.py         # タイトル自動生成
│
...
│   ├── test_mock_llm.py
│   ├── test_nodes.py
│   └── test_pdf_japanese.py
│
├── examples/
│   ├── __init__.py
│   └── example_usage.py               # 使用例（直接実行）
│
